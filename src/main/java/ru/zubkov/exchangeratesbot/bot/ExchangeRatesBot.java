package ru.zubkov.exchangeratesbot.bot;
...
public class ExchangeRatesBot extends TelegramLongPollingBot {
    ...
    private void cadCommand(Long chatId) {
        String formattedText;
        try {
            var cad = exchangeRatesService.getCADExchangeRate();
            var text = "Курс Канадского доллара на %s составляет %s рублей";
            formattedText = String.format(text, LocalDate.now(), cad);
        } catch (ServiceException e) {
            LOG.error("Ошибка получения курса доллара", e);
            formattedText = "Не удалось получить текущий курс доллара. Попробуйте позже.";
        }
    ...
    private void helpCommand(Long chatId) {
        var text = """
                ...
                Для получения курсов валют воспользуйтесь командами:
                /usd - курс доллара
                /eur - курс евро
                """;
